 ...
 * @approach: Use a vector of strings to collect characters for each row, traversing down and up as needed.
...
public:
  [[nodiscard]] std::string convert(const std::string& s, int numRows) const {
    if (numRows <= 1 || static_cast<size_t>(numRows) >= s.size()) return s;
    ...
    std::string result;
    for (const auto& r : rows) result += r;
    return result;
  }
};
