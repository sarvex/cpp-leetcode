...
public:
    [[nodiscard]] static auto countBattleships(const std::vector<std::vector<char>>& board) -> int {
        const auto m = static_cast<int>(board.size());
        const auto n = static_cast<int>(board[0].size());
        int ans = 0;
